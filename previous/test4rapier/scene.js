...
import { createCube, createBall, createCylinder,createCustomShape ,createArenaWalls,createBasketBallStand,createFootballStand,updatefootballStand} from './object.js';
...
export async function initTest2Scene(existingRenderer){
    ...
    const ball=createBall(gameScene,world,{position:{x:1,y:10,z:8},radius:0.22/2,mass:100,material:m});
...
export async function initGame2Scene(existingRenderer){
    ...
    const mw={ friction: 0.8, restitution: 0.1};
    player = createBall(gameScene,world, {position:{ x: 0, y: 5, z: 5 }, radius:0.5, mass:1000 ,material:{ friction: 0.5, restitution: 0.0},materialKey:'blue',type:'playerbody'});
    const ball=createBall(gameScene,world,{position:{x:1,y:10,z:0},radius:0.2,mass:100,material:m});
    pickableObjects.push(ball);
