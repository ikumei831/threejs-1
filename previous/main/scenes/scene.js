...
import { addMeshToPhysics, physicsObjects, meshPhysicsPair  ,initPhysics,eventQueue,handleMap,removeObject} from '../mesh/world.js';
import {createCloth,updateClothLines} from '../cloth.js';
import { createCube, createBall, createCylinder,createCustomShape ,createArenaWalls,createBasketBallStand,createFootballStand,updatefootballStand} from '../mesh/object.js';
...
export async function initRenderer(canvas) {
    ...
    if (renderer.xr) {
        renderer.xr.enabled = true; // 🟢 100% 成功解鎖
        console.log("WebXR 核心模組載入成功！");
    } else {
        console.error("此瀏覽器或環境不支援 renderer.xr，可能需降級至 WebGLRenderer");
    }
...
export async function initTestScene(existingRenderer) {
    ...
    player      = createBall(gameScene,world,{position:{ x: 0, y: 5, z: 5 },radius:0.5,mass:1000,material:{restitution:0},materialKey:"blue"});

    const ballList =[];
    const size=5;
    // for(let i=0; i<size;i++){
    //     for(let j=0;j<size;j++){
    //         ballList[i * size + j]=createBall(gameScene,world,{position:{x:i,y:10,z:j},radius:0.4,material:m,materialKey:'random'});
    //         pickableObjects.push(ballList[i * size + j]);
            
    //     }
    // }
    netaaa=createCloth(gameScene,world,{size:{r:10,c:10},space:0.8,materialKey:'white',type:'cloth',showGrid:true},true);
    const ball = createBall(gameScene,world,{position:{x:0,y:10,z:0},radius:0.5,mass:100,material:m});
   ...
   //console.log(meshPhysicsPair.get(floor).userData.id);
    ...
    playerBody = meshPhysicsPair.get(player);
    fpsCounter = new FPSCounter();
    physicsLinker = new PhysicsLinker(camera.game);
    interactionManager = new InteractionManager(camera.game, meshPhysicsPair, gameScene);
...
export async function initTest2Scene(existingRenderer){
    ...
    camera.game = new THREE.PerspectiveCamera(30, window.innerWidth / window.innerHeight, 0.1, 1000);
    camera.game.position.set(0,20,0);
    ...
    player = createBall(gameScene,world, {position:{ x: 1, y: 5, z: 10 }, radius:0.5, mass:1000 ,material:{ friction: 0.5, restitution: 0.0},materialKey:'blue',type:'playerbody'});

    const ball=createBall(gameScene,world,{position:{x:1,y:10,z:8},radius:0.32/2,mass:100,material:m});
    const defenser=createBall(gameScene,world,{position:{x:1,y:0.5,z:3},radius:0.5,mass:0,material:mw,materialKey:'blue',type:'defenser'});
    defenser.material.opacity=0.8;
    pickableObjects.push(ball);
    ...
    footballstand=createFootballStand(gameScene,world);
    ...
    interactionManager.throwForce=1;
...
export async function initGameScene(existingRenderer){
    ...
    player = createBall(gameScene,world, {position:{ x: 0, y: 5, z: 5 }, radius:0.2, mass:1000 ,material:{ friction: 0.5, restitution: 0.0},materialKey:'blue',type:'playerbody'});
    ...
    interactionManager.throwForce=10;
...
export async function initGame2Scene(existingRenderer){
    ...
    const mw={ friction: 0.8, restitution: 0.1};
    player = createBall(gameScene,world, {position:{ x: 0, y: 5, z: 5 }, radius:0.5, mass:10000 ,material:{ friction: 0.5, restitution: 0.0},materialKey:'blue',type:'playerbody'});
    const ball=createBall(gameScene,world,{position:{x:1,y:10,z:0},radius:0.22,mass:100,material:m});
    pickableObjects.push(ball);
...
export function animateGame(time,mode=1) {
    ...
    for (let i = 0; i < substeps; i++) {
        world.timestep = (1 / 30) / substeps;
        world.step(eventQueue);
        
        if (footballstand&&STATE.TESTING2) {
           //console.log(footballstand);
           updatefootballStand(footballstand);

        }
         if (STATE.TESTING) {
           //console.log(footballstand);
           updateClothLines(netaaa);
    ...
    eventQueue.drainCollisionEvents((h1, h2, started) => {
        if (started) {
            ...
            if (mesh1&&mesh2) {
                ...
                if(STATE.TESTING2){
                    if (type1 === 'defenser' && type2 === 'ball'||type1 === 'ball' && type2 === 'defenser'){
                        console.log("touched defenser");
                        
                    }

                    if (type1 === 'ball' && type2 === 'fnet'||type1 === 'fnet' && type2 === 'ball'){
                        worldevent.net=true;
    ...
    if(worldevent.net&&worldevent.floor&&STATE.TESTING2){
    ...
    playerControls.update(pointerLockControls,mode,renderer);
