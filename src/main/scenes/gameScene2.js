...
import { createBall, createCube, createFootballStand, updatefootballStand ,createFBXBall,createArenaWalls} from '../mesh/object.js';
...
export async function initGame2Scene(renderer, envMap) {
    ...
    let soccer;
     createFBXBall(
        ...
        {
            type: 'football',
            modelPath: 'texture/football.fbx', // 你的 FBX 檔案路徑
            position: { x: 1.75, y: 2, z: 13 },
            mass:10,
            material:{ friction: 0.05, restitution: 1.5 }
        ...
        (meshContainer) => {
            ...
            soccer=meshContainer;
            if (soccer) {
                soccer.userData.initialPosition = soccer.position.clone();
            }
        }
    );
    ...
    const targetMeshes = [player, floor, ...pickableObjects ,soccer,footballStand[0,1,2]]; // 這裡可以加入更多你想要調整材質的 Mesh[]
                ...
                eventQueue.drainCollisionEvents((handle1, handle2, started) => {
                    if (started) {
                        ...
                        if (mesh1 && mesh2) {
                            ...
                            if ((type1 === 'football' && type2 === 'fnet') || (type1 === 'fnet' && type2 === 'football')) worldevent.net = true;
                            if ((type1 === 'football' && type2 === 'floor') || (type1 === 'floor' && type2 === 'football')) if(worldevent.net)worldevent.floor = true;
                            //console.log(type1,type2,worldevent.net, worldevent.floor);
                        }
                    }
                });
            }
